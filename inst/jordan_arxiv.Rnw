...
\begin{abstract}
In this short article I introduce the {\tt jordan} package which
provides functionality for working with different types of Jordan
algebra.tensors, alternating forms, The package is available on CRAN
