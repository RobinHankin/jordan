...
Object `x` is a three-element vector, with each element having 15
degrees of freedom, as indicated by the row labelling.  Addition and
...
and `x` a scalar).  The distributive law is obeyed:

```{r}
x*(y+z) - (x*y + x*z)
```

We may observe that the resulting algebra is not associative:
...
So if we have $\mathcal{I},\mathcal{J},\mathcal{K}$ spin algebra
