...
```{r,label=verifyG8,cache=TRUE}
G8(rqhm(),rqhm(),rqhm())
G8(rspin(),rspin(),rspin())
```
...
```{r,cache=TRUE}
G8(ralbert(),ralbert(),ralbert())
```

## Numerical verification of $G_9$

```{r,label=defineH9G9}
H9 <- function(x,y,z){2*U(x)(z)*U(y*x-x*y)(U(z)(y*y)) - U(x)(U(z)(U(x*y-y*x)(U(y)(z))))}
G9 <- function(x,y,z){H9(x,y,z)-H9(y,x,z)}
```

Then $H_9()$ is identically zero for special Jordan algebras:

```{r,verifyH9equalszero,cache=TRUE}
H9(rqhm(),rqhm(),rqhm())
```

So the $G_9$ identity is automatically satisfied:
...
However, the Albert algebra does not:

```{r,albertH9G9,cache=TRUE}
G9(ralbert(),ralbert(),ralbert())
```
